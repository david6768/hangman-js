...
function easy() {
    ...
    while (lives > 0) {
        ...
        hiddennum = 0
        blankword = word
        while (blanknum < alphabet.length) {
            blankword = blankword.replace(alphabet[blanknum], '_')
            blanknum +=1
        }
        hiddenword = blankword
        
        while (hiddennum < guessed_letters.length) {
            if (word.includes(guessed_letters[hiddennum])) {
                wordlet = word.indexOf(guessed_letters[hiddennum])
                hiddenword = hiddenword[wordlet] + guessed_letters[hiddennum] + hiddenword[wordlet+1]
            hiddennum +=1
            }
        }
        ...
        if (word.includes(guess)) {
            ...
            guessed_letter = alphabet.indexOf(guess)
        }
        if (!(word.includes(guess))) {
            lives -= 1
            hang()
...
function setup() {
    console.log('[1] Easy')
    console.log('[2] Hard')
    difficulty = (prompt('Set Difficulty: '))
}
